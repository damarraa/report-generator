<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    <div x-data="mapPicker()" data-state-path="{{ $statePath }}" data-state="@json($state)"
        data-default-center="@json($defaultCenter)" wire:ignore
        class="w-full relative rounded-lg overflow-hidden z-0" style="height: 400px;">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[401] pointer-events-none">
            <svg class="w-8 h-8 text-red-500 drop-shadow-lg" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                viewBox="0 0 16 16">
                <path d="M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6z" />
            </svg>
        </div>

        <button type="button" x-on:click="getUserLocation"
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('mapPicker', () => ({
                    ...
                    async getUserLocation() {
                        if (!navigator.geolocation) return alert(
